    ...
    for freq, target_gain, target_phase, max_timeout in targets:
        try:
            result = subprocess.run(
                [sys.executable, "cmaes_worker.py", str(kp), str(ki), str(kd), str(g_ton), str(g_sc), str(g_phas), str(freq), str(target_gain), str(target_phase)],
            ...
            if result.returncode == 0 and result.stdout.strip():
                ...
                print(f"Eval CRASHED at {freq:.2f}Hz: Kp={kp:.1f}, Ki={ki:.1f}, Kd={kd:.1f}, Gton={g_ton:.2f}, Gsc={g_sc:.2f}, Gphas={g_phas:.2f}")
                return 100000.0 + np.random.rand() * 100.0
        except subprocess.TimeoutExpired:
            print(f"Eval TIMEOUT at {freq:.2f}Hz: Kp={kp:.1f}, Ki={ki:.1f}, Kd={kd:.1f}, Gton={g_ton:.2f}, Gsc={g_sc:.2f}, Gphas={g_phas:.2f}")
            return 100000.0 + np.random.rand() * 100.0
        except Exception as e:
            print(f"Eval ERROR at {freq:.2f}Hz: {e}")
            return 100000.0 + np.random.rand() * 100.0
            
    print(f"Eval SUCCESS: Kp={kp:.1f}, Ki={ki:.1f}, Kd={kd:.1f}, Gton={g_ton:.2f}, Gsc={g_sc:.2f}, Gphas={g_phas:.2f} => Bode Error: {total_sse:.1f}")
    return total_sse

if __name__ == "__main__":
    print("Starting BULLETPROOF Parallel Bode Plot CMA-ES Optimization (6 Parameters)...")
    # Parameters: Kp, Ki, Kd, G_ton, G_sc, G_phas
    x0 = [70.0, 40.0, 10.0, 2.0, 1.0, 0.5]
    sigma0 = 5.0 
    
    bounds = [
        [2.0, 0.0, 0.0, 0.1, 0.1, 0.1], 
        [500.0, 500.0, 50.0, 10.0, 5.0, 5.0]
    ]
    es = cma.CMAEvolutionStrategy(x0, sigma0, {'bounds': bounds, 'popsize': 16})
    
    # We use ThreadPoolExecutor because subprocess.run releases the GIL anyway.
    # 6 workers ensures we perfectly map to your 6 P-Cores without hitting RAM limits
